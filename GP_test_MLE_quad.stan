...
parameters {
  ...
  real<lower=0> alpha1;
  real<lower=0> alpha2;
  real<lower=0> sigma0; // These are normally fixed in the transformed data block, testing them as params
  real<lower=0> sigma1;
  real<lower=0> sigma2;
...
model {
  ...
  matrix[M, M] K2 = cov_exp_quad(x_zero, alpha2, rho[3]);
  ...
  target += multi_normal_cholesky_lpdf(to_vector(beta_2) | mu, L_K2);
  
  for (m in 1:M){
    for (n in 1:v_len[m]){
      target += (normal_lpdf(y_s[m][n] | beta_0[m] + beta_1[m] * X_s[m][n] + 0.5*beta_2[m] * (X_s[m][n]^2), sigma_y) * w_s[m][n]);
    }
  }
}
